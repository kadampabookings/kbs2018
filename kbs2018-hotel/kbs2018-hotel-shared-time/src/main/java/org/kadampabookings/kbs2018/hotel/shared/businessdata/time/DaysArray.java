...
public class DaysArray implements Iterable<Long> {
    ...
    public long getEpochDay(int index) {
        return TimeConverter.convertTime(getDay(index), timeUnit, TimeUnit.DAYS);
    }
    ...
    public DaysArray changeTimeUnit(TimeUnit newTimeUnit) {
        ...
        for (long day : array)
            newArray[i++] = TimeConverter.convertTime(day, timeUnit, newTimeUnit);
    ...
    public DaysArray shift(long shiftDays, TimeUnit shiftTimeUnit) {
        shiftDays = TimeConverter.convertTime(shiftDays, shiftTimeUnit, timeUnit);
        if (shiftDays == 0)
            return this;
        int n = array.length, i = 0;
        long[] newArray = new long[n];
